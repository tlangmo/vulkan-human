...
void Engine::init(int width, int height)
{
    ...
    auto result = builder.set_app_name("Vulkan Human")
    ...
    vkb::PhysicalDevice vkb_physical_device = selector.set_surface(m_surface).select().value();
     //   selector.set_minimum_version(1, 2).set_surface(m_surface).prefer_gpu_device_type().require_present(true).select().value();
